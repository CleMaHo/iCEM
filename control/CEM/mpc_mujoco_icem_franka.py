...
MAX_ITERATIONS = 100  # corresponds to approx. 30 seconds (with dt ~ 0.02)
print('Everythin loaded successfully.')
...
def forward_model_batch(x0, u_seq, model, dataset, dt=0.02, device=device):
    """
...
def icem_optimize(x_current, p_target, Q, R, horizon=20, control_dim=4, num_iterations=10,
                  N_init=200, elites_prev=None, K=20, y=1.25, sigma_init=1.0, min_sigma=0.05, beta_sigma=0.8, elitefrac=0.3):
    """
    ...
    return u_first, elites, final_cost
...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    for trial in range(NUM_TRIALS):
        ...
        for step in range(MAX_ITERATIONS):
            ...
            U_opt, iteration_elites, final_cost = icem_optimize(x_current, 
                p_target, 
                Q,R,
                horizon=20, 
                control_dim=4, 
            ...
            # U_opt, best_cost = icem_optimize(
            #     x0=x_current,
            #     p_target=p_target.to(device).flatten(),
            #     Q=Q,
            #     R=R,
            #     S=S,
            #     forward_model_batch=forward_model_batch,
            #     cost_function_batch=cost_function_batch,
            #     horizon=horizon,
            #     num_samples=NUM_SAMPLES,
            #     num_elites=ELITE_COUNT,
            #     max_iters=ICEM_ITERS,
            #     init_std=STD_DEV,
            #     min_std=0.02,
            #     y=1.25,
            #     goal_threshold=0.2,
